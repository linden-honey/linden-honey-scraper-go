...
func (s *defaultScraper) GetSong(ID string) (*domain.Song, error) {
	s.logger.Printf("Fetching song with id %s", ID)
	...
	s.logger.Printf(`Successfully fetched song with id %s and title "%s"`, ID, song.Title)
	return song, nil
}

// GetSongs returns a slice of pointers of the scrapped Song instances
func (s *defaultScraper) GetSongs() ([]*domain.Song, error) {
	s.logger.Println("Songs fetching started")
	...
	for _, p := range previews {
		...
		go func() {
			...
			if err != nil {
				s.logger.Println(errors.Wrap(err, "Error happened during song fetching"))
			} else {
				m.Lock()
	...
	s.logger.Println("Songs fetching successfully finished")
	return songs, nil
}
...
func (s *defaultScraper) GetPreviews() ([]*domain.Preview, error) {
	s.logger.Println("Previews fetching started")
	...
	s.logger.Println("Previews fetching successfully finished")
